            ...
            Ok(paginated_result) => {
                ...
                if items_count > 0 {
                    ...
                    for (i, equity) in paginated_result.items.iter().enumerate() {
                        println!("   {}. Symbol: {} | ID: {} | Active: {} | Description: {} | Instrument Type: {}", 
                            i + 1, 
                            equity.symbol.0, 
                            equity.id, 
                            equity.active,
                    ...
                    if pagination.total_items > 0 {
                        println!("⚠️  But API indicates {} total items exist!", pagination.total_items);
                    }
                }
                
                // Stop if we've gone past the total pages
                if page + 1 >= pagination.total_pages {
                    println!("🛑 Reached end of pages (page {} is the last page of {})", page, pagination.total_pages);
                    break;
                }
