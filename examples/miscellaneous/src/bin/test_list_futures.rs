...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !config.has_valid_credentials() {
        eprintln!("❌ No valid credentials found. Please set TASTYTRADE_USERNAME and TASTYTRADE_PASSWORD environment variables.");
        ...
        Ok(futures) => {
            ...
            if futures_count > 0 {
                ...
                for (i, future) in futures.iter().enumerate().take(10) {
                    println!("   {}. Symbol: {} | Product: {} | Exchange: {} | Active: {} | Expiration: {}", 
                        i + 1, 
                        future.symbol.0, 
                        future.product_code,
                        future.exchange,
    ...
    for product_code in popular_products {
        ...
        match tasty.list_futures(None::<&[&str]>, Some(product_code)).await {
            Ok(futures) => {
                println!("   ✅ Found {} futures for product {}", futures.len(), product_code);
                
                if !futures.is_empty() {
                    // Show first few futures for this product
                    for (i, future) in futures.iter().enumerate().take(3) {
                        println!("      {}. {} | Exp: {} | Active: {} | Tradeable: {}", 
                            i + 1, 
                            future.symbol.0,
                            future.expiration_date,
                            future.active,
    ...
    match tasty.list_futures(None::<&[&str]>, Some("ES")).await {
        Ok(es_futures) => {
            if !es_futures.is_empty() {
                ...
                println!("📊 Testing with specific symbols: {:?}", test_symbols.iter().map(|s| &s.0).collect::<Vec<_>>());
                
                match tasty.list_futures(Some(&test_symbols), None).await {
                    Ok(symbol_futures) => {
                        println!("   ✅ Found {} futures for specific symbols", symbol_futures.len());
                        for future in symbol_futures {
                            println!("      - {} | Product: {} | Exp: {} | Active: {}", 
                                future.symbol.0,
                                future.product_code,
                                future.expiration_date,
                                future.active
