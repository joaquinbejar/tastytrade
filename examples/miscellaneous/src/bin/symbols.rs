...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !positions.is_empty() {
        ...
        for (i, position) in positions.iter().enumerate() {
            ...
            if let tastytrade_rs::InstrumentType::Equity = position.instrument_type {
                debug!("Getting option chain for {}", position.symbol.0);
                match tasty.nested_option_chain_for(position.symbol.as_symbol()).await {
                    Ok(chain) => {
                        info!("    Available option expirations for {}:", position.symbol.0);
                        for (j, exp) in chain.expirations.iter().enumerate().take(5) {
                            info!("      {}. Expiration: {}, Days to expiry: {}, Strike count: {}", 
                                  j + 1, 
                                  exp.expiration_date, 
                            ...
                            if !exp.strikes.is_empty() {
                                ...
                                if exp.strikes.len() > 3 {
                                    info!("          ... and {} more strikes", exp.strikes.len() - 3);
                                }
                            }
                        }
                        if chain.expirations.len() > 5 {
                            info!("      ... and {} more expirations", chain.expirations.len() - 5);
                        }
                    },
                    Err(e) => {
                        info!("    Could not retrieve option chain for {}: {}", position.symbol.0, e);
                    }
                }
            }
        }
    ...
    for symbol in popular_symbols {
        ...
        match tasty.get_equity_info(&symbol).await {
            Ok(info) => {
                info!("  Equity symbol: {}, Streamer symbol: {}", info.symbol.0, info.streamer_symbol.0);

                // Try to get option chain for the symbol
                match tasty.nested_option_chain_for(symbol.as_symbol()).await {
                    Ok(chain) => {
                        info!("  Root symbol: {}, Shares per contract: {}", 
                             chain.root_symbol.0, 
                             chain.shares_per_contract);
