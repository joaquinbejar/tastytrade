        ...
        Ok(futures) => {
            ...
            if !futures.is_empty() {
                ...
                for (i, future) in futures.iter().enumerate().take(5) {
                    debug!("   {}. {} | Product: {} | Exp: {} | Exchange: {} | Active: {}", 
                        i + 1, 
                        future.symbol.0, 
                        future.product_code,
                    ...
                    if i < 2 {
                        ...
                        debug!("      - Notional Multiplier: {}", future.notional_multiplier);
                ...
                for future in &futures {
                    *product_count.entry(future.product_code.clone()).or_insert(0) += 1;
                }
        ...
        match tasty.list_futures(None::<&[&str]>, Some(product_code), None, None, None).await {
            Ok(futures) => {
                ...
                if !futures.is_empty() {
                    ...
                    debug!("   📊 {} analysis: {} active, {} active month", 
                        product_code, active, active_month);
                    
                    // Show first few futures for this product
                    for (i, future) in futures.iter().enumerate().take(3) {
                        debug!("      {}. {} - exp: {} (active: {}, tradeable: {})", 
                            i + 1, future.symbol.0, future.expiration_date, 
                            future.active, future.is_tradeable);
                    }
                }
            }
            Err(e) => {
                error!("❌ Error getting futures for product {}: {}", product_code, e);
            }
        }
    }
            ...
            if !es_futures.is_empty() {
                ...
                for future in es_futures.iter().take(2) {
                    match tasty.get_future(&future.symbol.0).await {
                        Ok(specific_future) => {
                            info!("✅ Retrieved future details for {}", specific_future.symbol.0);
                            debug!("   📊 Full details:");
                            ...
                            debug!("      - Notional Multiplier: {}", specific_future.notional_multiplier);
                            ...
                            debug!("      - Last Trade Date: {}", specific_future.last_trade_date);
                            debug!("      - Expiration Date: {}", specific_future.expiration_date);
                            debug!("      - Stops Trading At: {}", specific_future.stops_trading_at);
                            ...
                            debug!("      - Streamer Exchange Code: {}", specific_future.streamer_exchange_code);
                            debug!("      - Streamer Symbol: {}", specific_future.streamer_symbol.0);
                            debug!("      - Back Month First Calendar: {}", specific_future.back_month_first_calendar_symbol);
                            ...
                            debug!("      - Root Symbol: {}", specific_future.future_product.root_symbol.0);
                            debug!("      - Code: {}", specific_future.future_product.code);
                            debug!("      - Description: {}", specific_future.future_product.description);
                            debug!("      - Exchange: {}", specific_future.future_product.exchange);
                            debug!("      - Product Type: {}", specific_future.future_product.product_type);
                            debug!("      - Market Sector: {}", specific_future.future_product.market_sector);
                            debug!("      - Cash Settled: {}", specific_future.future_product.cash_settled);
                            debug!("      - Small Notional: {}", specific_future.future_product.small_notional);
                        }
                        Err(e) => {
                            error!("❌ Error getting future details for {}: {}", future.symbol.0, e);
                        }
                    }
                }
            } else {
