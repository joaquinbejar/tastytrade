        ...
        match tasty.list_futures_option_chains(product_code).await {
            Ok(options) => {
                if !options.is_empty() {
                    ...
                    info!(
                        "✅ Product {}: {} total options ({} calls, {} puts, {} active)",
                        product_code,
                        options.len(),
                        calls,
                        puts,
                    ...
                    info!(
                        "   📊 {} underlying symbols, {} expiration dates",
                        symbol_stats.len(),
                        expiration_stats.len()
                    );
            ...
            Ok(nested_chains) => {
                if !nested_chains.is_empty() {
                    ...
                    for (i, chain) in nested_chains.iter().take(2).enumerate() {
                        let total_chain_strikes: usize = chain
                            .option_chains
                            .iter()
                            .map(|oc| {
                                oc.expirations
                            ...
                            .sum();
                        let total_chain_expirations: usize = chain
                            .option_chains
                            .iter()
                            .map(|oc| oc.expirations.len())
                            .sum();
                        let futures_info = if !chain.futures.is_empty() {
                            &chain.futures[0].symbol
                        } else {
                            "N/A"
                        };
                        info!(
                            "      Nested Chain {}: {} expirations, {} strikes (Future: {})",
                            i + 1,
                            total_chain_expirations,
                            total_chain_strikes,
                            futures_info
                        );
                    }
