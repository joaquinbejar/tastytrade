...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match tasty.list_active_equities(0).await {
        Ok(paginated_equities) => {
            info!("✅ Retrieved {} active equities from page 1", paginated_equities.items.len());
            debug!("   📊 Pagination info:");
            debug!("      - Current page: {}", paginated_equities.pagination.page_offset);
            debug!("      - Items per page: {}", paginated_equities.pagination.per_page);
            debug!("      - Total pages: {}", paginated_equities.pagination.total_pages);
            debug!("      - Total items: {}", paginated_equities.pagination.total_items);
            ...
            if !paginated_equities.items.is_empty() {
                ...
                for (i, equity) in paginated_equities.items.iter().enumerate().take(5) {
                    ...
                    if i < 2 {
                        ...
                        debug!("      - Options Closing Only: {}", equity.is_options_closing_only);
                        debug!("      - Fractional Eligible: {}", equity.is_fractional_quantity_eligible);
                    }
                }
                
                if paginated_equities.items.len() > 5 {
                    debug!("   ... and {} more equities", paginated_equities.items.len() - 5);
                }
                
                // Analyze equity types
                let etfs = paginated_equities.items.iter().filter(|e| e.is_etf).count();
                let indices = paginated_equities.items.iter().filter(|e| e.is_index).count();
                let fractional_eligible = paginated_equities.items.iter().filter(|e| e.is_fractional_quantity_eligible).count();
                let closing_only = paginated_equities.items.iter().filter(|e| e.is_closing_only).count();
                ...
                match tasty.list_equities(&test_symbols).await {
                    Ok(specific_equities) => {
                        info!("✅ Retrieved {} equities for specific symbols", specific_equities.len());
                        
                        for equity in &specific_equities {
                            debug!("   📊 {}: {}", equity.symbol.0, equity.short_description);
                            debug!("      - Market: {} | ETF: {} | Active: {}", 
                ...
                for symbol in test_symbols.iter().take(3) {
                    ...
                    match tasty.get_equity(symbol).await {
                        Ok(equity) => {
                            ...
                            debug!("      - Market Time Collection: {}", equity.market_time_instrument_collection);
                ...
                for equity in &paginated_equities.items {
                    *market_count.entry(equity.listed_market.clone()).or_insert(0) += 1;
                }
                
                info!("✅ Found equities across {} different markets", market_count.len());
