        ...
        Ok(warrants) => {
            ...
            if !warrants.is_empty() {
                ...
                for (i, warrant) in warrants.iter().enumerate().take(5) {
                    info!(
                        "   {}. {} | Description: {} | Market: {} | Active: {} | Closing Only: {}",
                        i + 1,
                        warrant.symbol.0,
                        warrant.description,
                ...
                for warrant in &warrants {
                    *type_count
                        .entry(format!("{:?}", warrant.instrument_type))
                        .or_insert(0) += 1;
                }
                ...
                let test_symbols: Vec<_> = warrants
                    .iter()
                    .take(3)
                    .map(|w| w.symbol.0.as_str())
                    .collect();
                ...
                if !test_symbols.is_empty() {
                    match tasty.list_warrants(Some(&test_symbols)).await {
                        Ok(specific_warrants) => {
                            info!(
                                "✅ Retrieved {} specific warrants by symbols",
                                specific_warrants.len()
                            );

                            for warrant in &specific_warrants {
                                info!("   📊 {}: {}", warrant.symbol.0, warrant.description);
                                info!(
                                    "      - Market: {} | Active: {} | Closing Only: {}",
