...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        match tasty.get_equity_option(symbol).await {
            ...
            Err(e) => {
                ...
                if error_msg.contains("502 Bad Gateway") {
                    ...
                    error!(
                        "❌ Symbol not found: {} (this is expected for test symbols)",
                        symbol
                    );
                } else {
    ...
    match tasty.list_option_chains("AAPL").await {
        Ok(options) => {
            info!(
                "✅ Retrieved {} AAPL options using list_option_chains",
                options.len()
            );
            if !options.is_empty() {
    ...
    match tasty
        .list_equity_options(&specific_symbols, Some(true))
        .await
    {
        Ok(options) => {
            info!(
                "✅ Retrieved {} specific options using list_equity_options",
                options.len()
            );
