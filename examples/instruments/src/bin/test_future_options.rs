...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match tasty.list_future_products().await {
        Ok(products) => {
            ...
            for product in products.iter().take(3) {
                info!("🔍 Getting futures for product: {} ({})", product.code, product.description);
                
                match tasty.list_futures(None::<&[&str]>, Some(&product.code), None, None, None).await {
                    Ok(futures) => {
                        if !futures.is_empty() {
                            info!("✅ Found {} futures for product {}", futures.len(), product.code);
                            
                            // Add first few futures from this product
                            for future in futures.iter().take(2) {
                                verified_symbols.push(future.symbol.0.clone());
                                info!("   - Added future: {} (expires: {})", future.symbol.0, future.expiration_date);
                            }
                        } else {
    ...
    info!("✅ Using {} verified future symbols for testing", verified_symbols.len());
    
    // Demonstrate new list_futures parameters
    info!("\n🔧 Testing new list_futures parameters...");
    
    // Test with only active futures
    match tasty.list_futures(None::<&[&str]>, Some("MES"), None, Some(true), None).await {
        Ok(active_futures) => {
    ...
    match tasty.list_futures(None::<&[&str]>, Some("GE"), Some("CME"), None, None).await {
        Ok(cme_futures) => {
    ...
    info!("📊 Found {} unique product codes: {:?}", product_codes.len(), product_codes);
    
    let mut all_options = Vec::new();
    
    for product_code in &product_codes {
        info!("🔍 Getting futures option chains for product: {}", product_code);
        
        match tasty.list_futures_option_chains(product_code).await {
            Ok(options) => {
                info!("✅ Found {} future options for product {}", options.len(), product_code);
                all_options.extend(options);
            }
            Err(e) => {
                let error_msg = format!("{}", e);
                if error_msg.contains("404") || error_msg.contains("record_not_found") {
                    info!("ℹ️ No future options available for product {} (404 - not found)", product_code);
                } else {
                    error!("❌ Error getting future options for {}: {}", product_code, e);
                }
            }
        }
    }
    ...
    if options.is_empty() {
        ...
        info!(
            ...
            if !options.is_empty() {
                ...
                let primary_deliverable =
                    options.iter().filter(|o| o.is_primary_deliverable).count();
                ...
                for option in options.iter().take(3) {
                    match tasty.get_future_option(&option.symbol.0).await {
                        ...
                        Err(e) => {
                            let error_msg = format!("{}", e);
                            if error_msg.contains("404") || error_msg.contains("record_not_found") {
                                debug!("ℹ️ Future option {} not found (404) - may be expired or invalid", option.symbol.0);
                            } else {
