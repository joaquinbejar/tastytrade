...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let crypto_symbols = vec![
        "BTC/USD", "ETH/USD", "LTC/USD", "BCH/USD", "DOGE/USD", "ADA/USD", "SOL/USD",
    ];

    match tasty.list_cryptocurrencies(&crypto_symbols).await {
        Ok(cryptos) => {
            info!(
                "✅ Found {} cryptocurrencies from {} requested symbols",
                cryptos.len(),
                crypto_symbols.len()
            );
            ...
            if !cryptos.is_empty() {
                ...
                for (i, crypto) in cryptos.iter().enumerate().take(5) {
                    debug!(
                        "   {}. {} | Description: {} | Active: {} | Closing Only: {}",
                        i + 1,
                        crypto.symbol.0,
                        crypto.description,
                        crypto.active,
                    ...
                    if i < 2 {
                        ...
                        if !crypto.destination_venue_symbols.is_empty() {
                            debug!(
                                "      - Destination Venues: {}",
                                crypto.destination_venue_symbols.len()
                            );
                ...
                for crypto in cryptos.iter().take(3) {
                    match tasty.get_cryptocurrency(&crypto.symbol.0).await {
                        Ok(specific_crypto) => {
                            ...
                            info!(
                                "      - Streamer Symbol: {}",
                                specific_crypto.streamer_symbol.0
                            );
                            info!(
                                "      - Venues: {} destination venues",
                                specific_crypto.destination_venue_symbols.len()
                            );

                            // Show first few venues
                            for (i, venue) in specific_crypto
                ...
                for crypto in &cryptos {
                    for venue in &crypto.destination_venue_symbols {
                        *venue_count
                            .entry(venue.destination_venue.clone())
                            .or_insert(0) += 1;
                        total_venues += 1;
                    }
                }
                ...
                info!(
                    "✅ Found {} total destination venues across {} unique venues",
                    total_venues,
                    venue_count.len()
                );
